...
The `backup_containers.sh` script is designed to create consistent backups of your Docker containers. It temporarily halts the containers to avoid inconsistencies during the backup process, ensuring a reliable snapshot is created. Once the backup is complete, all containers are restarted.
